...
var properties = [];
var readFileContents = fs.readFileSync(__dirname+'/1231Run.csv');
...
//	var date = element[2];
	...
	zillow.callApi('GetSearchResults', addressParam)
	  .then(function(searchResultsResponse) {
	  	...
	  	fs.appendFileSync(__dirname+'/output.csv', addressParam.address+',');
...
//		output.street = searchResult.address[0].street[0];
		fs.appendFileSync(__dirname+'/output.csv', searchResult.address[0].street[0]+',');
...
//		output.zipcode = searchResult.address[0].zipcode[0];	
		fs.appendFileSync(__dirname+'/output.csv', searchResult.address[0].zipcode[0]+',');
	// 	output.currentZestLow = parseInt(searchResult.zestimate[0].valuationRange[0].low[0]._);
		fs.appendFileSync(__dirname+'/output.csv', searchResult.zestimate[0].valuationRange[0].low[0]._+',');
	// 	output.currentZest = parseInt(searchResult.zestimate[0].amount[0]._);
		fs.appendFileSync(__dirname+'/output.csv', searchResult.zestimate[0].amount[0]._+',');
	// 	output.currentZestHigh = parseInt(searchResult.zestimate[0].valuationRange[0].high[0]._);
		fs.appendFileSync(__dirname+'/output.csv', searchResult.zestimate[0].valuationRange[0].high[0]._+',');
	// 	output.currentZestDate = searchResult.zestimate[0]["last-updated"][0];
		fs.appendFileSync(__dirname+'/output.csv', searchResult.zestimate[0]["last-updated"][0]+'\015');
		console.log(searchResult.address[0].street[0]+" = "+searchResult.zestimate[0].amount[0]._);
	// 	inspect(output);
...
//				    	fs.appendFileSync(__dirname+'/output.csv', searchResult.zpid[0]+','+zestFullHistory[index].y+',');
